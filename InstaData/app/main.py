...
def call_openai_llm(
    ...
    """
    Generic helper for calling OpenAI Chat API
    """
...
def suggestQuestion(description):
    ...
    st.write("Suggested question generated...", response)
...
def getChartCode(prompt):
    ...
    # Optional cleanup if the model wraps code in ```python
...
def getBusinessAnalysis(prompt):
    ...
    return call_openai_llm(
        system_prompt=systemPrompt,
        user_prompt=prompt,
        temperature=0.4,  # slight creativity for analysis
    )
...
def assembleDictionaryParts(parts):
    ...
    predictions_response = call_openai_llm_1(
        system_prompt=systemPrompt,
        user_prompt="\n".join(parts),
        temperature=0.4
    )
...
def display_csv_explore_tab(tab):
    with tab:
        ...
        try:
            ...
            st.write("I had trouble creating the data dictionary.")
            pass
