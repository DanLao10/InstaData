...
def setup_sidebar():
    with st.sidebar:
        ...
        st.write("Welcome to :violet[InstaData], your virtual data analyst!")
        ...
        st.sidebar.header(":violet[InstaData Chatbot]")
        question = st.sidebar.text_input(
            "Ask InstaData Chatbot any question!")
        if st.sidebar.button("Ask"):
            answer = chatbot._llm_reply(question, st.secrets.datarobot_credentials.END_POINT, st.secrets.datarobot_credentials.API_KEY)
    ...
    while attempts < max_retries:
        ...
        try:
            if csv_mode:
                ...
                st.session_state["sqlCode"], st.session_state["results"] = executeSnowflakeQuery(st.session_state["prompt"], user, st.session_state["password"],
...
def read_svgs_and_generate_html_report():
    st.session_state["datarobot_logo_svg"] = read_svg_as_base64("../assets/DataRobotLogo.svg")
    st.session_state["customer_logo_svg"] = read_svg_as_base64("../assets/small_square_placeholder.svg")
...
def mainPage():
    ...
    # log in page
    if st.session_state.signedout == False and st.session_state.signout == False:
        with st.sidebar:
            st.image("logo.png", width=300)
        ...
        # st.markdown(""" :violet[InstaData helps you quickly get insights from your data!]""")
